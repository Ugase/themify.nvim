...
local function parse_repository(repository)
    if repository:sub(1, 8) == 'https://' then
        ...
        Utilities.error(#chunks > 3 or (chunks[1]:len() == 0 or chunks[2]:len() == 0),
            { 'Themify: Invalid repository name: "', repository, '"' })

        return { source = repository, author = chunks[2], name = chunks[3] }
    else
        local chunks = vim.split(repository, '/')

        Utilities.error(#chunks > 2 or (chunks[1]:len() == 0 or chunks[2]:len() == 0),
            { 'Themify: Invalid repository name: "', repository, '"' })
...
function M.add_colorscheme(colorscheme_source, colorscheme_info)
    ...
    Utilities.error(M.colorschemes_data[colorscheme_id] ~= nil,
        { 'Themify: Duplicate colorscheme: "', colorscheme_id, '"' })
...
function M.install_colorscheme(colorscheme_id)
    ...
    if colorscheme_data.status == 'not_installed' then
        ...
        local pipeline = Pipeline:new({
            Tasks.clone(Data.colorschemes_path, colorscheme_data.repository.source,
                table.concat({ colorscheme_data.repository.author, colorscheme_data.repository.name }, '-'),
            ...
            if error ~= nil then
                ...
                if local_commit == remote_commit then
                    ...
                    pipeline:start(function(code, _, stderr)
                        ...
                        if code == 0 then
                            colorscheme_data.info = table.concat({ 'Updated ', local_commit:sub(0, 7), ' -> ',
                                remote_commit:sub(0, 7) })
                        else
...
function M.check_colorscheme_commit(colorscheme_id, callback)
    ...
    local pipeline = Pipeline:new({
        ...
        Tasks.get_commit(colorscheme_data.path, table.concat({ 'origin', colorscheme_data.repository.branch }, '/'),
...
function M.count_colorscheme_amount()
    ...
    for i = 1, #M.colorschemes_id do
        ...
        if colorscheme_data.type == 'remote' then
            ...
            M.colorschemes_amount[colorscheme_status] = M.colorschemes_amount[colorscheme_status] == nil and 1 or
                M.colorschemes_amount[colorscheme_status] + 1
        elseif colorscheme_data.type == 'local' then
            M.colorschemes_amount['local'] = M.colorschemes_amount['local'] == nil and 1 or
                M.colorschemes_amount['local'] + 1
        end
    end
end
